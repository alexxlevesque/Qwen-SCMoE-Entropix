...
# def _scmoe_dbg(msg: str):
#     # Print only if SCMOE_DEBUG is set. Respect DDP ranks if present.
#     if not os.environ.get("SCMOE_DEBUG"):
#         return
...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def forward(
        ...
        if len(routed_tok) == 1 and routed_tok[0] == 128:
            ...
            rand_cap = min(128, self.num_experts)
        ...
        for expert_idx in range(self.num_experts):
            ...
            if top_x.shape[0] == 0:
                continue  # skip experts that received no tokens (small runtime win)
            ...
            # We need to make sure to multiply the output hidden states by `routing_weights`
            # on the corresponding tokens (top-1 and top-2)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_states, _ = self.self_attn(
            ...
            **kwargs,  # keep attention kwargs unchanged (same structure as original)
        )
        ...
        # SCMoE: pass runtime routing knobs to sparse MoE block using kwargs.get(...)
        ...
        if isinstance(hidden_states, tuple):
            hidden_states, _ = hidden_states  # ignore router_logits here
...
class Qwen3MoeModel(Qwen3MoePreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        # ---------------- SCMoE additions (match Mixtral SCMoE) ----------------
        # Runtime routing controls forwarded to decoder layers / MoE blocks.
...
class Qwen3MoeForCausalLM(Qwen3MoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # SCMoE parity: expose default routed_tok from config so callers can override or use defaults
        self.routed_tok = getattr(config, "routed_tok", None)   
    ...
    def forward(
        ...
        # ---------------- SCMoE additions (match Mixtral SCMoE) ----------------
        # Runtime routing controls forwarded to the base model / decoder layers / MoE blocks.
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        outputs: MoeModelOutputWithPast = self.model(
            ...
            cache_position=cache_position,
            # --- NEW: pass-through with fallback to config / existing top_k ---
