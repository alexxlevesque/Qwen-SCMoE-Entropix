...
class StopOnSubstrings(StoppingCriteria):
    def __init__(self, tokenizer, stop_strings, start_pos: int):
        ...
        self.start_pos = start_pos  # index where generated text begins

    def __call__(self, input_ids, scores, **kwargs):
        # decode only what the model has generated (exclude the original prompt)
...
def generate(rank, args):
    ...
    for start in tqdm(range(0, len(prompt_lst), args.batch_size), disable=rank != 0):
        ...
        # turn "\n" literals into real newlines (same list you used for the stopper)
        raw_list = [s.strip() for s in (args.stop_text.split(",") if args.stop_text else [])]
        ...
        if stop_strings:
            ...
            generation_text = [_trim_at_stops(t, stop_strings) for t in generation_text]
        #------------------------------------------------------------------------------
